...
public class makeSect2 : MonoBehaviour
{
    ...
    public void gen(float currRight, int sliceNum, float sliceLength, int levelNum)
    {
        ...
        walkStart = relativeRight - 2.5f;
        walkEnd = relativeRight + 2.5f;
        Debug.Log("level gen 2 done: " + count + " starting at: " + currRight);
    }
    ...
    public void generatePowerUps(int levelNum)
    {
        ...
        if (genPowerUp)
        {
            ...
            //code to generate collectable at spawn point as child of currentBlock
            newPU = Instantiate(powerUp, new Vector3(relativeRight + 2, 1, 5), Quaternion.identity);
    ...
    public void generateOP()
    {
        ...
        if (genOnePunch)
        {
            newOP = Instantiate(op, new Vector3(relativeRight, 1, 5), Quaternion.identity);
